...
export default function LandingNavbar() {
  ...
  return (
    <nav className={cn(
      ...
      isScrolled ? "bg-[#0A0A0A]/95 backdrop-blur-3xl border-b border-saffron/30 py-4 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.8)]" : "bg-transparent"
    )}>
      <div className="container mx-auto px-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-4 group">
          ...
          <div className="flex flex-col -space-y-1">
            <span className="text-2xl font-black tracking-[0.1em] text-white uppercase italic">
        ...
        <div className="hidden lg:flex items-center gap-10">
          {['The Hub', 'The Tribe', 'Routes', 'Support'].map((item) => (
            <a 
              ...
              className="text-xs font-black uppercase tracking-widest text-white/50 hover:text-saffron transition-all"
            >
              {item}
            </a>
          ))}
        ...
        <div className="flex items-center gap-6">
          {user ? (
            ...
            <>
              <Link 
                to="/login" 
                className="hidden sm:flex items-center gap-2 text-xs font-black uppercase tracking-widest text-white/70 hover:text-white transition-colors"
              >
                Sign In
              </Link>
              <button 
